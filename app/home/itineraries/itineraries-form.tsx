...
export default function CreateOrUpdateItineraryScreen() {
  const { session } = useSession();
  const router = useRouter();
  const params = useLocalSearchParams<{ itineraryData?: string }>();
  const isUpdate = !!params.itineraryData;
  ...
  useEffect(() => {
  if (isUpdate && params.itineraryData) {
    try {
      ...
      if (data.planDaily && Array.isArray(data.locations)) {
        ...
        setLocations([]); // <-- Prevents .map of undefined
      }
    } catch (err) {
      setErrorMessage('Failed to load itinerary data for update.');
    }
  }
}, [isUpdate, params.itineraryData]);
  ...
  const handleSubmit = () => {
    ...
    (async () => {
      ...
      if (isUpdate && params.itineraryData) {
        // Update
        const data = JSON.parse(params.itineraryData);
        saveResult = await updateItineraryApi(data.id, result);
      } else {
        // Create
        saveResult = await saveItinerary(result);
      }
      setLoading(false);
  ...
  return (
    <ThemedView color='primary' style={{ flex: 1 }}>
      <Header label={isUpdate ? "Update Itinerary" : "Create Itinerary"} />
      ...
      >
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <View style={{paddingHorizontal: 20, marginBottom: 10}}>
            ...
            <Button
              title={isUpdate ? "Update Itinerary" : "Create Itinerary"}
      ...
      <LoadingModal
        ...
        success={success}
        successMessage={isUpdate ? "Itinerary updated!" : "Itinerary saved! Redirecting..."}
